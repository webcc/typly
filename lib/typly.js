...
module.exports = {
    ...
    isInstanceOf: function (_object, _class)
    {
        this.assertObject(_object);
        this.assertNotNull(_class);
        this.assertFunction(_class);
        if(Symbol.hasInstance){
            // not yet implemented in V8/nodejs
            return _object instanceof _class;
        }
        else{
            if(_object instanceof _class){
                // this always work - the opposite not
                return true;
            }
            // fallback for multiple instances of module inside node_modules
